...
def dir_to_filelist(dir_path):
    ...
    assert all([os.path.basename(p1).split('.')[0] == os.path.basename(p2).split('.')[0] for p1, p2 in
                zip(txt_paths, wav_paths)])
    ...
    with open(os.path.join(dir_path, 'filelist.csv'), 'w+') as g:
        g.writelines(lines)


if __name__ == '__main__':
    # use this code one time to create the filelists in the an4 dataset dir
